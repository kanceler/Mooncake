...
MasterService::MasterService(const MasterServiceConfig& config)
    ...
    for (size_t i = 0; i < object_type_reuse_scales_.size(); ++i) {
        const auto& score_policy =
            config.object_type_eviction_score_policies[i];
        const auto& budget_policy = config.object_type_eviction_policies[i];
        ValidateObjectTypeEvictionScorePolicy(score_policy);
        ValidateObjectTypeEvictionPolicy(budget_policy);
        object_type_reuse_scales_[i] = score_policy.reuse_scale;
        object_type_soft_pin_weights_[i] = score_policy.soft_pin_weight;
        object_type_eviction_graces_[i] = score_policy.eviction_grace;
        object_type_budget_ratios_[i] = budget_policy.budget_ratio;
    }
...
void MasterService::BatchEvict(double evict_ratio_target,
    ...
    auto adjusted_age = [this, rank_reference_time](
        ...
        const double soft_pin_weight =
            object_type_soft_pin_weights_[idx];
        ...
        const int64_t expired_age = std::max<int64_t>(
            std::chrono::duration_cast<std::chrono::milliseconds>(
                ...
                eviction_grace,
            0);
        const double weight = is_soft_pinned ? soft_pin_weight : 1.0;
    ...
    for (auto& v : local_candidates) {
        for (auto& candidate : v) {
            ...
            const auto data_type_idx =
                static_cast<uint8_t>(candidate.data_type);
    ...
    auto execute_candidate = [&](const Candidate& c,
                                 bool allow_soft_pinned) -> EvictionResult {
        ...
        if (it == tenant_state.metadata.end()) return {};
        ...
        if (!it->second.IsGrouped()) {
            PublishKvRemovedAfterEvict(c.key, evict_result.freed_bytes, "cpu",
    ...
    // First pass: Assign by data type, then evict inside over-budget scopes.
    if (!candidates.empty()) {
        ...
        if (total_mem_capacity > 0) {
            for (size_t data_type_idx = 0;
                ...
                if (static_cast<double>(type_used_bytes) <=
                    type_budget_bytes) {
                    continue;
                }
                ...
                type_evict_num = std::min(
                    type_evict_num, (long)type_candidate_indices.size());
                if (type_evict_num <= 0) {
                    continue;
                }
                ...
                const int64_t target_adjusted_age =
                ...
                for (const auto candidate_idx : type_candidate_indices) {
                    ...
                    auto evict_result =
                        execute_candidate(c, /*allow_soft_pinned=*/false);
    ...
    // Global fallback to the original target. This still uses the type-aware
    // score so fallback does not collapse back to raw lease time.
    ...
    if (target_evict_num > 0) {
        if (target_evict_num <= static_cast<long>(no_pin_objects.size())) {
            ...
            std::nth_element(no_pin_objects.begin(),
                             ...
                             no_pin_objects.end(), std::greater<int64_t>());
            ...
            std::nth_element(
                ...
                soft_pin_objects.end(), std::greater<int64_t>());
