...
void InitMasterConf(const mooncake::DefaultConfig& default_config,
    ...
    for (size_t type_id = 0; type_id < mooncake::kObjectDataTypeCount;
         ++type_id) {
        const auto idx = type_id;
        const std::string type_key = std::to_string(type_id);

        bool has_score_policy = false;
        auto score_policy =
            master_config.object_type_eviction_score_policies[idx];
        ...
        if (TryGetDoubleConfig(
                ...
                &budget_ratio)) {
            mooncake::ObjectTypeEvictionPolicy policy;
            policy.budget_ratio = budget_ratio;
            if (mooncake::ValidateObjectTypeEvictionPolicy(policy)) {
                master_config.object_type_eviction_policies[idx] = policy;
            }
        }
    }
